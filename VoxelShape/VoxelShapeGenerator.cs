...
namespace GraffitiEntertainment.VoxelShape
{
    public static class VoxelShapeGenerator
    {
        ...
        public static List<Vector3> GenerateShape(ShapeType type, Vector3 shapeSize, Vector3 voxelSize,
            Vector3 rotation = default, Texture2D image = null, string spriteName = null, Rect spriteRect = default)
        {
            switch (type)
            {
                case ShapeType.Cube:
                    return GenerateCube(shapeSize, voxelSize, rotation);
                case ShapeType.Sphere:
                    return GenerateSphere(shapeSize, voxelSize);
                case ShapeType.Wedge:
                    return GenerateWedge(shapeSize, voxelSize, rotation);
                case ShapeType.FromImage:
                    return GenerateFromImage(shapeSize, voxelSize, rotation, image, spriteName, spriteRect);
                default:
        ...
        private static List<Vector3> GenerateCube(Vector3 shapeSize, Vector3 voxelSize, Vector3 rotation)
        {
            var cells = new List<Vector3>();
            if (voxelSize.x <= 0 || voxelSize.y <= 0 || voxelSize.z <= 0) { /* Error */ return cells; }
            ...
            foreach (int x_idx in GetCenteredIndexRange(shapeSize.x, voxelSize.x))
            {
                foreach (int y_idx in GetCenteredIndexRange(shapeSize.y, voxelSize.y))
                {
                    foreach (int z_idx in GetCenteredIndexRange(shapeSize.z, voxelSize.z))
                    {
                        // Calculate the center position of this potential voxel
                        Vector3 voxelCenterPos = new Vector3(x_idx * voxelSize.x, y_idx * voxelSize.y, z_idx * voxelSize.z);
        ...
        private static List<Vector3> GenerateSphere(Vector3 shapeSize, Vector3 voxelSize)
        {
            var cells = new List<Vector3>();
            if (voxelSize.x <= 0 || voxelSize.y <= 0 || voxelSize.z <= 0) { /* Error */ return cells; }
            shapeSize = new Vector3(Mathf.Max(0, shapeSize.x), Mathf.Max(0, shapeSize.y), Mathf.Max(0, shapeSize.z));
        ...
        private static List<Vector3> GenerateWedge(Vector3 shapeSize, Vector3 voxelSize, Vector3 rotation)
        {
            var cells = new List<Vector3>();
            if (voxelSize.x <= 0 || voxelSize.y <= 0 || voxelSize.z <= 0) { /* Error */ return cells; }
            shapeSize = new Vector3(Mathf.Max(0, shapeSize.x), Mathf.Max(0, shapeSize.y), Mathf.Max(0, shapeSize.z));
        ...
        private static List<Vector3> GenerateFromImage(Vector3 shapeSize, Vector3 voxelSize, Vector3 rotation,
            Texture2D image, string spriteName, Rect spriteRect)
        {
            ...
            Vector3Int gridCount = Vector3Int.CeilToInt(new Vector3(
                nonNegativeShapeSize.x / voxelSize.x,
                nonNegativeShapeSize.y / voxelSize.y,
                nonNegativeShapeSize.z / voxelSize.z
            ));
            ...
            Vector3 totalGridVolumeSize = Vector3.Scale(gridCount, voxelSize);
            Vector3 centerOffset = (totalGridVolumeSize - voxelSize) / 2.0f;
                ...
                for (int z = 0; z < gridCount.z; z++) // Image maps to XZ plane
                {
                    ...
                    if (isOpaque)
                    {
                        ...
                        for (int y = 0; y < gridCount.y; y++)
                        {
                            ...
                            Vector3 cornerRelativePos = new Vector3(x * voxelSize.x, y * voxelSize.y, z * voxelSize.z);
        ...
        private static IEnumerable<int> GetCenteredIndexRange(float shapeSizeAxis, float voxelSizeAxis)
        {
            if (voxelSizeAxis <= 0) yield break; // Avoid division by zero

            int targetVoxelCount = Mathf.RoundToInt(shapeSizeAxis / voxelSizeAxis);
